          ...
          {jarProducts.map((p) => (
            <Link key={p.id} to={`/products/${p.seoSlug || p.id}`} className="group rounded-2xl bg-white p-4 ring-1 ring-gold/15 transition hover:shadow-lg">
              <div className="aspect-square overflow-hidden rounded-xl bg-cream-dark/40 p-2">
                <img src={p.image} alt={`${p.name} wholesale manufacturer China - factory direct low MOQ`} width={800} height={800} className="h-full w-full object-contain object-center transition duration-500 group-hover:scale-105" loading="lazy" />
              </div>
